...
To understand why VLMs frequently fail to capture spatial relationships, we must look at their architectural foundation: the **Transformer**. Since its introduction in *Attention is All You Need*<d-cite key="vaswani2017attention"></d-cite>, Transformer has become the backbone of modern language and vision-language models due to the power of its core building block, **self-attention**. In brief, self-attention calculates the relevance of one token (word) to another. For example, in the sentence, "*Here is a yellow chick*," the method identifies that "*yellow*" is a descriptor highly relevant to "*chick*."
